...
def main():
    ...
    if args.spectrain:
        ...
        logging.info('Using spectrain v2')
        if args.square:
            if args.log_dir != None:
...
def train(train_loader, r, optimizer, epoch):
    ...
    for i in range(n - num_warmup_minibatches):
        ...
        if is_last_stage():
            ...
            # logging.info(f'prec1[0] = {prec1[0][0]}, loss = {loss}')
            losses.update(loss.item(), output.size(0))
            top1.update(prec1[0][0], output.size(0))
...
def validate(val_loader, r, epoch):
    ...
    with torch.no_grad():
        ...
        if is_last_stage():
            logging.info(' * Prec@1 {top1.avg:.2f}%'.format(top1=top1))
