...
class StageRuntime:
    ...
    def initialize(self, model, inputs_module_destinations, configuration_maps,
        ...
        if module_to_stage_map is None:
            ...
            if stage_to_depth_map is not None:
                self.num_warmup_minibatches = stage_to_depth_map[str(
                    self.stage)]
            else:
                self.num_warmup_minibatches = self.num_ranks - 1
                for i in range(self.stage):
                    self.num_warmup_minibatches -= len(stage_to_rank_map[i])
        ...
        for i in range(len(modules)):
            if group is not None:
                if ((i < (len(modules) - 1) and self.is_criterion) or
                        not self.is_criterion):
                    num_parameters += \
        ...
        if self.comm_handler is not None:
            self.comm_handler.initialize(
                self.receive_ranks, self.send_ranks, self.tensor_tags,
                self.target_tensor_names, self.training_tensor_dtypes,
                self.rank_in_stage, self.num_ranks_in_stage,
                self.ranks_in_previous_stage, self.ranks_in_next_stage)
    ...
    def train(self, num_iterations):
        ...
        if self.comm_handler is not None:
            ...
            self.comm_handler.start_helper_threads(num_iterations,
                                                   forward_only=False)
    ...
    def receive_tensors_forward(self):
        ...
        if self.loader_iter is not None:
            ...
            if self.model_type == TRANSLATION:
                ...
                self.tensors[-1]["target"] = tgt[1:].cuda().contiguous().view(
                    -1)
        ...
        for i, (module, input_names, output_names) in \
            ...
            if i == (len(modules) - 1) and self.is_criterion:
                ...
                if self.model_type == SPEECH_TO_TEXT:
                    ...
                    module_outputs = [
                        module(output, target, output_sizes, target_sizes) /
                        input0_size[0]
                    ]
                else:
                    module_outputs = [
                        module(tensors[input_name], tensors["target"])
            ...
            for (output_name, module_output) in zip(output_names,
                                                    module_outputs):
    ...
    def run_backward(self):
        ...
        for (input_names, output_names) in zip(all_input_names,
                                               all_output_names):
            for input_name in input_names:
            ...
            for output_name in output_names:
                if output_name not in all_input_names_set:
                    if output_name not in self.gradients:
                        ...
                        output_gradients[output_name] = self.gradients[
                            output_name]
        ...
        torch.autograd.backward(
            tuple([outputs[output_name] for output_name in outputs]),
            grad_tensors=tuple(
                [output_gradients[output_name] for output_name in outputs]))
